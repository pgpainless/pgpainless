...
class KeyRingReader {
    ...
    fun keyRing(inputStream: InputStream): PGPKeyRing? = readKeyRing(inputStream)

    /**
     * Read a [PGPKeyRing] (either [PGPSecretKeyRing] or [PGPPublicKeyRing]) from the given byte
     * array.
     *
     ...
     */
    @Throws(IOException::class)
    fun keyRing(bytes: ByteArray): PGPKeyRing? = keyRing(bytes.inputStream())

    /**
     * Read a [PGPKeyRing] (either [PGPSecretKeyRing] or [PGPPublicKeyRing]) from the given ASCII
     * armored string.
     *
     ...
     */
    @Throws(IOException::class)
    fun keyRing(asciiArmored: String): PGPKeyRing? = keyRing(asciiArmored.toByteArray(UTF8))

    @Throws(IOException::class)
    fun publicKeyRing(inputStream: InputStream): PGPPublicKeyRing? = readPublicKeyRing(inputStream)

    @Throws(IOException::class)
    fun publicKeyRing(bytes: ByteArray): PGPPublicKeyRing? = publicKeyRing(bytes.inputStream())
    ...
    @Throws(IOException::class)
    fun secretKeyRing(inputStream: InputStream): PGPSecretKeyRing? = readSecretKeyRing(inputStream)

    @Throws(IOException::class)
    fun secretKeyRing(bytes: ByteArray): PGPSecretKeyRing? = secretKeyRing(bytes.inputStream())
    ...
    companion object {
        ...
        @JvmStatic val UTF8: Charset = charset("UTF8")

        /**
         * Read a [PGPKeyRing] (either [PGPSecretKeyRing] or [PGPPublicKeyRing]) from the given
         * [InputStream]. This method will attempt to read at most <pre>maxIterations</pre> objects
         * from the stream before aborting. The first [PGPPublicKeyRing] or [PGPSecretKeyRing] will
         * be returned.
         *
         * @param inputStream inputStream containing the OpenPGP key or certificate
        ...
        /**
         * Read a public key ring from the provided [InputStream]. If more than maxIterations PGP
         * packets are encountered before a [PGPPublicKeyRing] is read, an [IOException] is thrown.
         *
        ...
        /**
         * Read a public key ring collection from the provided [InputStream]. If more than
         * maxIterations PGP packets are encountered before the stream is exhausted, an
         * [IOException] is thrown. If the stream contain secret key packets, their public key parts
        ...
        @JvmOverloads
        @Throws(IOException::class)
        fun readPublicKeyRingCollection(
            inputStream: InputStream,
            maxIterations: Int = MAX_ITERATIONS
        ...
        /**
         * Read a secret key ring from the provided [InputStream]. If more than maxIterations PGP
         * packets are encountered before a [PGPSecretKeyRing] is read, an [IOException] is thrown.
         *
         * @param inputStream input stream
         * @param maxIterations max iterations before abort
         * @return public key ring
        ...
        /**
         * Read a secret key ring collection from the provided [InputStream]. If more than
         * maxIterations PGP packets are encountered before the stream is exhausted, an
        ...
        @JvmOverloads
        @Throws(IOException::class)
        fun readSecretKeyRingCollection(
            inputStream: InputStream,
            maxIterations: Int = MAX_ITERATIONS
        ...
        @JvmStatic
        @Throws(IOException::class)
        fun readKeyRingCollection(
            inputStream: InputStream,
            isSilent: Boolean
        ): PGPKeyRingCollection = PGPKeyRingCollection(inputStream, isSilent)
    }
}
